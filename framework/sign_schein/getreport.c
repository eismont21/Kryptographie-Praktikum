...
static void Modify_Message(Message *msg) {
...
int main(int argc, char **argv) {
    ...
    Modify_Message(&new_msg);
    Forge_Message(&msg, &new_msg);
    Generate_MDC(&new_msg, p, mdc);

    printf("Gefälschte Nachricht von mir:\n");
    for (int i = 0; i < new_msg.body.VerifyRequest.NumLines; i++) {
        printf("\t%s\n", new_msg.body.VerifyRequest.Report[i]);
    }

    Transmit(con, &new_msg, sizeof(new_msg));
    ReceiveAll(con, &msg, sizeof(msg));
